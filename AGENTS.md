...
- 在 `E:\Github` 下做新任务或新项目时，默认先使用一个清晰独立的 project folder；不要把脚本、临时输出、CSV/JSON/log/report 等产物直接写到 `E:\Github` 根目录。
- 如果已有合适项目目录，优先复用；如果没有，先创建明确命名的目录，再在其中组织 `scripts/`、`logs/`、`outputs/`、`reports/`、`data/` 等子目录。
- 复杂、多步骤或会长时间运行的脚本必须先落盘为 `.py`、`.ps1` 等真实文件，再用指定解释器运行；禁止把长脚本硬塞进 PowerShell here-string、`python -` 或其他不可复现的一次性命令体。
- 探针脚本也应放在项目内，例如 `scripts/` 或 `outputs/tmp/` 对应的临时脚本目录；除非确实是一行级别的简单命令。
- 长任务或数据敏感任务必须保留可运行脚本、参数/配置、日志和输出 manifest，方便复盘、审计和重新运行。
